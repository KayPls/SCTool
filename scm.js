function Init(friendMessage, checkBlocked, debug) {
	...
	const APP_NAME = "Social Club Tool";
	const APP_NAME_SHORT = "SCT";
	const APP_AUTHOR = "Kay";
	const APP_LINK = "https://github.com/KayPls/SCTool";
	const APP_LINK_ISSUES = "https://github.com/KayPls/SCTool/issues/new";
	const APP_LINK_SC = "https://" + window.location.host;
	const APP_CLIENT_VERSION = localStorage.getItem('SCT_CLIENT_VERSION');
			...
			try {
				...
				if (userNickname != "" && isLoggedIn) {
					...
					if (APP_CLIENT_VERSION != APP_VERSION) {
						...
						localStorage.setItem('SCT_CLIENT_VERSION', APP_VERSION);
					}
